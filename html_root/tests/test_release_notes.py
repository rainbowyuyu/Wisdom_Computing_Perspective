...
def test_release_notes_navigation(origin,path,kind,width):
 with sync_playwright() as p:
  ...
  assert page.locator('#docs-version-select').input_value()=='update-v-0.4.6'
  page.wait_for_function("document.querySelector('[id=\"update-v-0.4.6\"]').getBoundingClientRect().top>document.querySelector('.docs-update-jump').getBoundingClientRect().bottom")
  assert page.locator('.docs-release-latest').get_by_text('安全性与使用速度优化').count()==1
  headings=page.locator('.docs-release h2').all_text_contents()
  assert len(headings)==30
  assert headings[0].strip()=='v 0.4.6' and headings[-1].strip()=='初始版'
  ...
  page.wait_for_function("document.querySelector('#docs-version-select').value==='update-v-0.4.6'")
  Path('tests/artifacts').mkdir(exist_ok=True)
