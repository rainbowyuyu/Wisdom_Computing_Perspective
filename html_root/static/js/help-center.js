...
export const FAQ=[
    ...
    ['tasks','复杂题目如何拆解？多个任务能一起进行吗？','复杂题可转交智能体拆解，明确条件、子问题和依赖后逐步生成。互不依赖的子任务可并行，有依赖的步骤按顺序执行，服务繁忙时排队。可在智能体任务列表查看进度、停止任务，或只继续未完成部分；缺少条件时需要补充题面。切换页面不等于取消后台任务，服务重启后中断的任务可手动继续。'],
    ...
    ['ocr','识别结果不准确，或手写题目无法识别怎么办？','支持常见代数、矩阵及微积分表达式等，但光线、连笔与题目结构会影响识别准确性。建议使用清晰 JPG / PNG 图片，保持字迹间距，上传图片不超过 5 MB。识别后先核对完整题面、数字、上下标和条件，可在识别结果中编辑公式或修改 LaTeX，再转入解题。'],
