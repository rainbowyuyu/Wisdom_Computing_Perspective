...
async function perform(step,context,signal,retry=false){
    ...
    if(!context.toolDone.has(step)){
        if(step.section==='calculate'){
            ...
            if(step.recognition){
                await navigate('detect',signal);
                await waitFor(()=>document.getElementById('detect-problem'),signal);
                const source=state.lastRequest?.image_base64;
                if(source?.startsWith('data:image/')){
                    const [header,encoded]=source.split(',');
                    const bytes=Uint8Array.from(atob(encoded),c=>c.charCodeAt(0));
                    const transfer=new DataTransfer();
                    transfer.items.add(new File([bytes],'question.png',{type:header.slice(5).split(';')[0]}));
                    const input=document.getElementById('image-upload');
                    document.querySelectorAll('#detect .tab-btn').forEach(button=>{if(button.textContent.includes('上传'))button.click();});
                    if(input){input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));}
                }
                window.dispatchEvent(new CustomEvent('recognition-result',{detail:step.recognition}));
                state.messages.push({role:'assistant',text:'已整理图片题面，请在识别页核对后再开始计算。'});
                state.result={section:'detect',label:'核对识别题面'};
                context.toolDone.add(step);return;
            }
